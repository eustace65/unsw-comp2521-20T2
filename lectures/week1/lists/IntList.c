...
void freeIntList (IntList L)
{
	...
	for (curr = L->first; curr != NULL; curr = next) {
		next = curr->next;
		free (curr);
	}
...
#else
/** Release all resources associated with an IntList. */
void doFreeList(struct IntListNode *n)
{
	if (n == NULL)
		return;
	else {
		doFreeList (n->next);  // cleanup tail of list
		free (n);              // free head
	}
}
void freeIntList (IntList L)
...
/** Display an IntList as one integer per line to a file.
 * Assume that the file is open for writing. */
void doPrint(FILE *f, struct IntListNode *n)
{
	if (n == NULL)
		return;
	else {
		fprintf(f, "%d\n", n->data); // print head of list
		doPrint(f, n->next);         // print rest of list
	}
}
void IntListPrint (FILE *outf, IntList L)
{
	// TODO: recursive version
	doPrint(outf, L->first);
}
#endif
