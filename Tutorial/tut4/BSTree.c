...
// Scroll to the very bottom for tut04 stuff :)
...
// Written by Dylan Blecher in June, 2019
// d.blecher@unsw.edu.au

// Count how many internal nodes there are
...
// Count how deep the node with value "key" is.
// "how deep" means how many rows below the root
// return -1 if this node doesn't exist
int nodeDepth(BSTree t, int key) {
	if (t == NULL) return -1;
	if (t->value == key) return 0;
	if (key > t->value) {
		int rightDepth = nodeDepth(t->right, key);
		if (rightDepth == -1) return -1;
		return rightDepth + 1;
	} else if (key < t->value) {
		int leftDepth = nodeDepth(t->left, key);
...
// Find the MAXIMUM branch length (number of edges)
...
// What is height? Number of levels ("rows") in the tree.
static int height(BSTree t) {
	if (t == NULL) return 0;
...
// Print the height difference between the left & right subtree
// of every node in a given binary tree.
// What is height? Number of levels ("rows") in the tree.
// Return the height of a given binary tree
void printHeightDiff(BSTree t) {
	if (t == NULL) return;

	int lh = height(t->left);
	int rh = height(t->right);

	printf("difference = %d (lh = %d, rh = %d)\n", abs(lh - rh), lh, rh);

	printHeightDiff(t->left);
	printHeightDiff(t->right);
}

int printHeightDiff2(BSTree t) {
	if (t == NULL) return 0;

	int lh = printHeightDiff2(t->left);
	int rh = printHeightDiff2(t->right);
	
	int diff = abs(lh - rh);
	
	printf("Height difference at node %d = %d\n", t->value, diff);
	printf("(Left height = %d, right height = %d)\n\n", lh, rh);

	return 1 + (lh > rh ? lh : rh);
}

// If it's an AVL tree, returns height of AVL tree
// Otherwise, returns -1
int isAVL(BSTree t) {
	if (t == NULL) return 0;

	int lh = isAVL(t->left);
	int rh = isAVL(t->right);
