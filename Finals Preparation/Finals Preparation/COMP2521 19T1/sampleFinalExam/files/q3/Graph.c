...
// HELPER FUNCTION:
static int compareC (Connects a, Connects b);
...
static int compareC (Connects a, Connects b) {
    int ans = 1; // it is in order - no need to do anything
    if (a.nconn < b.nconn) { // needs to be in descending order of conns
        ans = 0;
    } else if ((a.nconn == b.nconn) && (a.vertx > b.vertx)) { // if equal cons. vertices in asc order
        ans = 0;
    }
    return ans;
}
